// Based on https://andreasmb.github.io/lever-jobs-embed/index.js
...
window.loadLeverJobs = function (options) {
    ...
    // Modified 9/2020 by CF for AppLovin & Partner Studios (Add support for limiting by department in API call)
    var url = "https://api.lever.co/v0/postings/" + options.accountName + "?department=" + options.department + "&group=team&mode=json";
        ...
        for(var i = 0; i < groupedPostings.length; i++) {
            ...
            for (j = 0; j < groupedPostings[i].teams.length; j ++) {
                ...
                for (var k = 0; k < groupedPostings[i].teams[j].postings.length; k ++) {
                    content += "<a class=\"lever-job-link-wrap\" data-department=\"" + groupedPostings[i].departmentTitle +"\" data-team=\"" + groupedPostings[i].teams[j].postings[k].categories.team + "\" data-location=\"" + groupedPostings[i].teams[j].postings[k].categories.location + "\" target=\"_blank\" rel=\"noreferrer\" href=\"" + groupedPostings[i].teams[j].postings[k].hostedUrl + "\"><li class=\"lever-job\" data-department=\"" + groupedPostings[i].departmentTitle +"\" data-team=\"" + groupedPostings[i].teams[j].postings[k].categories.team + "\" data-location=\"" + groupedPostings[i].teams[j].postings[k].categories.location + "\">" +
          "<span class=\"lever-job-title\">" + sanitizeForHTML(groupedPostings[i].teams[j].postings[k].text) + "</span><span class=\"lever-job-tag\">" + (sanitizeForHTML(groupedPostings[i].teams[j].postings[k].categories.location) || "") + "</span><div class=\"lever-job-arrow\"></div></li></a>";
                }
